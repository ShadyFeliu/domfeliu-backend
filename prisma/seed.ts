...
async function main(): Promise<void> {
  ...
  if (trackCount === 0) {
    const tracksFile = await fs.readFile(
  ...
  if (eventCount === 0) {
    ...
    const events = eventsData.map((e: any, i: number) => ({
      ...e,
  ...
  const venueCount = await (prisma as any).venue.count();
  if (venueCount === 0) {
    const venuesFile = await fs.readFile(
      path.join(__dirname, 'data/venues.json'),
      'utf8',
    );
    const venues = JSON.parse(venuesFile);
    await (prisma as any).venue.createMany({ data: venues });
